...
const preview = {
  ...
  neighbors: function (where, id) {
    ...
    if (where === 'deck') {
      ...
      /* iterate collection */
      const index = sort.getSortIndex(filter.apply(collection), sort.mode)
  ...
  reload: function () {
    ...
    for (const [name, deck] of Object.entries(decks.data)) {
      ...
      frame.ondragover = function (ev) {
        if (!decks.drag || !collection[decks.drag]) { return }
        ev.preventDefault()
      }
      ...
      for (const [id, place] of Object.entries(deck)) {
        const index = collection[id].name
        decksize = decksize + 1

        if (!decklist[index]) {
          decklist[index] = { count: 1, id: id }
          cmcindex.push({ key: index, cmp: collection[id].cmc })
        } else {
      ...
      for (let i = 0; i < cmcindex.length; i++) {
        ...
        const isLand = collection[id].types.toLowerCase().includes('land')

        const card = document.createElement('div')
        card.style.backgroundColor = convert.colorStringToHex(collection[id].color)
        ...
        label.innerHTML = collection[id].name_loc ? collection[id].name_loc : collection[id].name
        label.onclick = function () { preview.show(card, id) }
        card.appendChild(label)

        const mana = document.createElement('span')
        mana.classList = 'deck-card-mana'
        mana.innerHTML = convert.parse(collection[id].mana)
        card.appendChild(mana)

        /* add cmc calc */
        const cost = collection[id].cmc || 0
        if (cost < 1 && isLand) {
          if (!cmc['99']) { cmc['99'] = 0 }
          cmc['99'] = cmc['99'] + count
        } else {
...
const view = {
  repaint: function () {
    ...
    const results = filter.apply(collection)
...
/* initialize viewport */
  ...
  for (const key in collection) {
    if (typeof collection[key].frame === 'undefined') {
      console.log('Creating card frame for ' + key)
      collection[key].frame = view.createCard(key, collection[key])
      console.log(collection[key].frame )
      collection[key].frame.parent = collection[key]
    }
    // viewport.appendChild(collection[key].frame)
    viewport.append(collection[key].frame)
  }
